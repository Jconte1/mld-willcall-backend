...
exports.publicOrderReadyRouter.get("/:orderNbr", async (req, res) => {
    ...
    if (shouldRefreshDetails) {
        ...
        if (noticeBaid) {
            try {
                ...
                const result = await (0, fetchOrderLastModified_1.fetchOrderLastModified)(noticeBaid, orderNbr, restService);
    ...
    const lineWhere = readyInventoryIds.size === 0
        ? {
            orderNbr,
    ...
    let lines = await prisma_1.prisma.erpOrderLine.findMany({
        where: lineWhere,
        select: lineSelect,
