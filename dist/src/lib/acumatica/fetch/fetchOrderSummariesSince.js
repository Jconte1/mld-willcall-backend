...
async function fetchOrderSummariesSince(restService, baid, { sinceLiteral, pageSize: pageSizeArg, maxPages: maxPagesArg, useOrderBy = false, } = {}) {
    ...
    for (let page = 0; page < maxPages; page++) {
        ...
        params.set("$filter", [`CustomerID eq '${baid.replace(/'/g, "''")}'`, `LastModified ge ${since}`].join(" and "));
        params.set("$select", select);
        params.set("$custom", custom);
        if (useOrderBy)
            params.set("$orderby", "LastModified desc");
        ...
        all.push(...rows);
        console.log(`[fetchOrderSummariesSince] baid=${baid} page=${page} size=${pageSize} rows=${rows.length} ms=${ms} truncated=${rows.length === pageSize}`);
