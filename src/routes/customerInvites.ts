...
const REQUEST_INVITE_BODY = z.object({
  baid: z
    .string()
...
const CREATE_INVITE_BODY = z.object({
  userId: z.string().min(1),
  baid: z
    .string()
    .transform((v) => v.replace(/\s+/g, "").toUpperCase())
...
const REVOKE_INVITE_BODY = z.object({
  userId: z.string().min(1),
  baid: z
    .string()
    .transform((v) => v.replace(/\s+/g, "").toUpperCase())
...
const UPDATE_ROLE_BODY = z.object({
  userId: z.string().min(1),
  baid: z
    .string()
    .transform((v) => v.replace(/\s+/g, "").toUpperCase())
...
const REMOVE_MEMBER_BODY = z.object({
  userId: z.string().min(1),
  baid: z
    .string()
    .transform((v) => v.replace(/\s+/g, "").toUpperCase())
...
function resolveInviteRecipient(
  email?: string | null,
  { allowTestOverride = true }: { allowTestOverride?: boolean } = {}
) {
...
async function hasAdminForBaid(baid: string) {
...
async function getAccess(userId: string, baid: string) {
  const user = await prisma.users.findUnique({
    where: { id: userId },
    select: { id: true, isDeveloper: true },
  });
  if (!user) return { user: null, role: null, isDeveloper: false };

  if (user.isDeveloper) return { user, role: "ADMIN", isDeveloper: true };
...
async function ensureAdmin(userId: string, baid: string) {
  const access = await getAccess(userId, baid);
  if (!access.user) return { ok: false, status: 404 };
  if (access.isDeveloper) return { ok: true, access };
  if (access.role !== "ADMIN") return { ok: false, status: 403 };
  return { ok: true, access };
}

async function ensureMember(userId: string, baid: string) {
  const access = await getAccess(userId, baid);
  if (!access.user) return { ok: false, status: 404 };
  if (access.isDeveloper) return { ok: true, access };
  if (!access.role) return { ok: false, status: 403 };
  return { ok: true, access };
}
...
async function revokePendingInvites(baid: string) {
...
async function assignAdminIfNeeded(baid: string) {
...
customerInvitesRouter.post("/request", async (req, res) => {
  ...
  if (ipKey) await recordAttempt(ipKey, verified);
...
customerInvitesRouter.post("/invitations/list", async (req, res) => {
  ...
  if (!access.ok) return res.status(access.status ?? 403).json({ message: "Not authorized" });
...
customerInvitesRouter.post("/invitations", async (req, res) => {
...
customerInvitesRouter.post("/invitations/revoke", async (req, res) => {
...
customerInvitesRouter.post("/members/list", async (req, res) => {
  const parsed = LIST_BODY.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ message: "Invalid request body" });
...
customerInvitesRouter.post("/members/role", async (req, res) => {
...
customerInvitesRouter.post("/members/remove", async (req, res) => {
...
customerInvitesRouter.post("/requests/list", async (req, res) => {
  const parsed = LIST_BODY.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ message: "Invalid request body" });
