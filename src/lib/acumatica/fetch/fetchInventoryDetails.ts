...
export default async function fetchInventoryDetails(
  ...
  orderNbrs: string[],
  {
    batchSize = Number(process.env.LINES_BATCH_SIZE || 16),
    pool = Number(process.env.LINES_POOL || 4),
    maxSockets = Number(process.env.LINES_MAX_SOCKETS || 8),
    retries = Number(process.env.LINES_RETRIES || 4),
    maxUrl = Number(process.env.ACUMATICA_MAX_URL || 7000),
    timeoutMs = Number(process.env.LINES_TIMEOUT_MS || 25000),
    minDelayMs = Number(process.env.LINES_MIN_DELAY_MS || 150),
  }: {
    batchSize?: number;
    pool?: number;
    maxSockets?: number;
  ...
  const fetchBatchAdaptive = async (
    batch: string[],
    batchIndex: number,
    totalBatches: number,
    depth = 0
    ...
    const attemptFetch = async (attempt: number): Promise<void> => {
      ...
      if (!resp.ok) {
        ...
        if (splitWorthy && batch.length > 1) {
          console.warn(
            `[fetchInventoryDetails] split batch (status=${status}) urlLen=${url.length} depth=${depth} baid=${baid} size=${batch.length}`
          );
          const mid = Math.floor(batch.length / 2);
          await fetchBatchAdaptive(batch.slice(0, mid), batchIndex, totalBatches, depth + 1);
          await fetchBatchAdaptive(batch.slice(mid), batchIndex, totalBatches, depth + 1);
        ...
        if ((status === 429 || (status >= 500 && status < 600)) && attempt < retries) {
          ...
          console.warn(
            `[fetchInventoryDetails] retry ${attempt + 1}/${retries} baid=${baid} batch=${
              batchIndex + 1
            }/${totalBatches} status=${status} wait=${wait}ms`
          );
          await sleep(wait);
      ...
      all.push(...rows);
      console.log(
        `[fetchInventoryDetails] baid=${baid} batch=${batchIndex + 1}/${totalBatches} depth=${depth} orders=${batch.length} rows=${rows.length} ms=${ms}`
      );
    };
  ...
  await poolRun(chunks, Math.max(1, pool), async (batch, idx) =>
    fetchBatchAdaptive(batch, idx, chunks.length)
  );
  console.log(
    `[fetchInventoryDetails] baid=${baid} totalRows=${all.length} batches=${chunks.length} pool=${pool} batchSize=${batchSize}`
  );
  return all;
}
