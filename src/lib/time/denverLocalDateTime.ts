...
function getDenverOffsetForDate(dateStr: string) {
...
export function parseDenverDateOnly(dateStr: string) {
  const offset = getDenverOffsetForDate(dateStr);
  return new Date(`${dateStr}T12:00:00${offset}`);
}

export function makeDenverDateTime(dateStr: string, time: string) {
  const offset = getDenverOffsetForDate(dateStr);
